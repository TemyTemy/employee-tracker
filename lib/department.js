...
const DELETE_DEPARTMENT_SQL = 'delete from department WHERE id = ?';
...
const doDeleteDepartment = (id, callBack) => {
  const connection = dbutil.connect();
  connection.query(DELETE_DEPARTMENT_SQL, [id], (err, results, fields) => {
    if (err) {
      return console.error(err.message);
    }   
...
const department = {
    ...
    deleteDepartment: (id, callBack) => {
        doDeleteDepartment(id, callBack);
    }
}
