...
const FETCH_EMPLOYEE_BY_DEPARTMENT_EXTENDED_SQL = `select e.id employee_id,
                                         e.first_name first_name,
                                         e.last_name last_name,
                                         r.id role_id, r.title title,
                                         em.id manager_id,
...
const FETCH_EMPLOYEE_BY_MANAGER_EXTENDED_SQL = `select e.id employee_id,
                                         e.first_name first_name,
                                         e.last_name last_name,
                                         r.id role_id, r.title title,
                                         em.id manager_id,
...
const FETCH_EMPLOYEE_SALARY_BY_DEPARTMENT_EXTENDED_SQL = `select d.name department_name, SUM(r.salary) salary
                                         from employee e 
                                         join role r on r.id = e.role_id
                                         join department d on d.id = r.department_id                                        
                                         WHERE d.id = ?`;                                         
...
const DELETE_EMPLOYEE_SQL = 'delete FROM employee WHERE id = ?';
let MANAGER_LIST = [];
let ROLES_LIST = [];
var connection;
...
const doDeleteEmployee = (id, callBack) => {
    const connection = dbutil.connect();
    connection.query(DELETE_EMPLOYEE_SQL, [id], (err, results, fields) => {
      if (err) {
        return console.error(err.message);
      }   
...
const doListEmployeeByDepartment = (cb, departmentId) => {  
    connection = doConnect();         
    connection.query(FETCH_EMPLOYEE_BY_DEPARTMENT_EXTENDED_SQL, [departmentId], (err, res) => {
        result = [];
        res.forEach(({employee_id, manager_id, first_name, last_name, title, manager_first_name, manager_last_name, department_name, salary }) => {
            result.push({
                        "id": employee_id,
...
const doListEmployeeSalaryByDepartment = (cb, departmentId) => {  
    connection = doConnect();         
    connection.query(FETCH_EMPLOYEE_SALARY_BY_DEPARTMENT_EXTENDED_SQL, [departmentId], (err, res) => {
        result = [];
        res.forEach(({ department_name, salary }) => {
            result.push({                    
                        "department": department_name,
                        "salary": salary
                    });
...
const doListEmployeeByManager = (cb, managerId) => {  
    connection = doConnect();         
    connection.query(FETCH_EMPLOYEE_BY_MANAGER_EXTENDED_SQL, [managerId], (err, res) => {
        result = [];
        res.forEach(({employee_id, manager_id, first_name, last_name, title, manager_first_name, manager_last_name, department_name, salary }) => {
            result.push({
                        "id": employee_id,
...
const employee = {
    ...
    listEmployeesByDepartment: (deptId, callBack) => {
        doListEmployeeByDepartment(callBack, deptId);
    },
    getEmployeeSalaryByDepartment: (deptId, callBack) => {
        doListEmployeeSalaryByDepartment(callBack, deptId);
    },
    listEmployeesByManager: (managerId, callBack) => {
        doListEmployeeByManager(callBack, managerId);
    },
    ...
    updateExistingEmployee: (ans, id, callBack) => {
        doUpdateEmployee(ans.firstName, ans.lastName, ans.role, ans.manager, id, callBack)
    },
    doDeleteEmployee: (id, callBack) => {
        doDeleteEmployee(id, callBack);
    }
}
